...
def get_args():
    ...
    parser.add_argument('--session_cnt', default=10, type=int)
...
def train(args):
    ...
    if not args.llm_init:
        model_name = '/vanilla.pth'
    elif args.session and args.profile:
        model_name = f'/final_{args.beta}.pth'
    elif args.session:
        model_name = f'/session.pth'
    elif args.profile:
        model_name = f'/profile.pth'
    else:
        model_name = f'/llm_init.pth'
    ...
    with open(f'trained/{dataname}/result.txt', 'a') as f:
        if args.llm_init:
            f.write(f'{model_name} {args.beta} {ndcg:.4f} {hr:.4f} {ndcg_20:.4f} {hr_20:.4f}\n')
